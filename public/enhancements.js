(() => {
  ...
  const warningsTab = $("#warningsTab");
  const profileTab = $("#profileTab");
  const profile = $("#userProfile");
  const warnings = $("#warningsView");
  if (warningsTab && profileTab && profile && warnings) {
    warningsTab.addEventListener("click", async () => {
    ...
    profileTab.addEventListener("click", () => { profile.hidden = false; warnings.hidden = true; profileTab.classList.add("active"); warningsTab.classList.remove("active"); });
  }
  ...
  if (drawButton && drawModal && canvas) {
    ...
    drawButton.addEventListener("click", () => drawModal.hidden=false);
    $("#closeDraw").addEventListener("click", () => drawModal.hidden=true);
    ...
    $("#saveDrawing").addEventListener("click", () => {
      ...
      const preview=$("#drawingPreview"); preview.innerHTML=`<img src="${drawingData}" alt="Drawing preview"><button id="removeDrawing" class="postButton" type="button">Remove Drawing</button>`; preview.hidden=false;
      $("#removeDrawing").onclick=()=>{drawingData=null;preview.hidden=true;preview.innerHTML="";}; drawModal.hidden=true;
    });
    const form=$("#postForm");
    form.addEventListener("submit", async e => {
      ...
      e.preventDefault();
      const u=getUser(); if(!u)return;
      const fd=new FormData(); fd.append("userId",u.id); fd.append("text",$("#postText").value.trim());
      const blob=await (await fetch(drawingData)).blob(); fd.append("image",blob,"greenverse-drawing.png");
      const r=await fetch("/api/posts",{method:"POST",body:fd}),d=await r.json();
      if(!r.ok){alert(d.error||"Could not create post.");return;}
      $("#postText").value=""; drawingData=null; $("#drawingPreview").hidden=true; $("#drawingPreview").innerHTML=""; $("#counter").textContent="0 / 500"; location.reload();
    }, true);
  }
})();
